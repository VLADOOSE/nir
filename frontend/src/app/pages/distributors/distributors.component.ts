...
@Component({
  ...
  template: `
    ...
    <table>
      ...
      <tbody>
        <tr *ngFor="let d of distributors">
          <td>{{ d.name }}</td>
          <td>{{ d.inn }}</td>
          <td>{{ d.contact }}</td>
          <td class="actions">
...
export class DistributorsComponent implements OnInit {
  ...
  onSave() {
    ...
    const req = this.editingId
      ? this.api.update('distributors', this.editingId, body)
      : this.api.create('distributors', body);
    req.subscribe(() => {
      this.showForm = false;
      this.loadData();
    });
  ...
  onDelete(id: number) {
    if (confirm('Удалить?')) {
      this.api.delete('distributors', id).subscribe(() => this.loadData());
    }
  }
}
