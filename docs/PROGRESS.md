...
**Последнее обновление:** 2026-07-12 (конец сессии). Ветка `main`, HEAD `f8f6530`. Всё ниже — **уже в main**.
...
1. ~~ч.3b авто-парс~~ ✔ + ~~выбор победителя~~ ✔ + ~~воронка стадий~~ ✔ + ~~канал/площадка + кнопка «Открыть»~~ ✔ + ~~фильтр «Площадка»~~ ✔ + ~~авто-импорт СК-Фармации (fms.ecc.kz HTML-скрейп Jsoup, пакет integration/skpharmacy)~~ ✔ (2026-07-10/11; живой прогон: 8 device-тендеров с реального портала, 0 ошибок) + ~~единая кнопка «Обновить тендеры» (обе площадки по фильтру)~~ ✔ + ~~разбор ТЗ СК-Фармации (кнопка «ТЗ» на SK-лотах: documents→modal→PDF, ключ `source_lot_code` V12)~~ ✔ (2026-07-12; живой прогон 521464-1: КТ+Ангиограф ТЗ разобраны с реального портала, join по коду). **СЛЕДУЮЩЕЕ БОЛЬШОЕ — подача заявки после сбора цен** (оба канала goszakup + СК-Ф): пакет = ценовое предложение + форма соответствия тех.спец + обеспечение заявки (1% суммы) + квал.требования (портал авто) + **ЭЦП**; АИС держит цену/модель/РУ/ТЗ → предзаполняет, подпись+отправка (криптосокет) скорее вручную оператором. Детали процесса — у отца. Не строить вслепую до консультации.
