...
> ⚠️ **Заливочный и текстовый токены НЕ взаимозаменяемы — это две разные роли, а не синонимы.** Заливочный как цвет текста даёт **2,1–3,4:1** в светлой теме (так и уехало в прод: чипы стадий 1,91:1, «★ победитель» 2,54:1 — чинили задачей 5b). Текстовый как заливка сделает тинт мутным и грязным. В светлой теме это РАЗНЫЕ значения; в тёмной они совпадают (`--danger-text` == `--danger` и т.д.) — именно поэтому дефект и не был виден в тёмной теме, ради которой всё затевалось. Правило простое: смотри на слот CSS, а не на исходный хекс. `color:` → текстовый токен; `background`, `border-color`, `border-left`, `box-shadow`, заливка шкалы, любой `color-mix(...)` для подложки → заливочный.
