...
11. **Не трогать шаблоны (`template:`) и TypeScript-логику.** Задачи меняют только содержимое блока `styles: [` … `]`. Если кажется, что нужно поправить разметку — это сигнал остановиться и написать об этом в отчёте, а не менять.
    ...
    **Единственное исключение — инлайн-цвета в разметке** (найдено в Task 8). `[style.color]`, `[style.background]`, `style="color:…"`, `[ngStyle]` перебивают таблицу стилей, поэтому такие места из `styles` не чинятся вообще. Живой пример: колонка «Реестр» в `private-requests` держала `[style.color]="… ? '#065f46' : '#6b7280'"` и в тёмной теме давала 2,1:1 — нечитаемо. Правило: **найдя инлайн-цвет, перенеси его в класс, ничего больше в разметке не меняя** (условия, интерполяции, `data-label` — не трогать), и оставь комментарий у нового CSS-правила. Перед сдачей задачи прогони по своим файлам:
    ```bash
    grep -nE '\[style\.(color|background)|style="[^"]*(color|background)|\[ngStyle\]' <файл>
    ```
12. **⚠️ Обратные кавычки в CSS-комментариях ломают сборку** (наступили в Task 8). Блок `styles: [` … `]` — это шаблонный литерал JS, и любой символ `` ` `` внутри комментария закрывает его досрочно. Angular ругается непрозрачно: `Failed to resolve styles at position 0 to a string`, на CSS-ошибку не похоже. В комментариях внутри `styles` использовать «ёлочки» или простые кавычки, не бэктики.
...
| `#e5e7eb`, `#d1d5db`, `#eee`, `#f0f0f0` — **заливка / рамка** | `var(--border)` |
| `#d1d5db`, `#e5e7eb` — **цвет текста** (разделители, очень бледные подписи) | `color-mix(in srgb, var(--text-muted) 45%, transparent)` |
| `#f3f4f6`, `#f9fafb` | `var(--surface-2)` |
| `#fff`, `#ffffff` | `var(--surface)` — если это **фон**; `var(--accent-contrast)` — если это **текст на цветной кнопке** |
| `#1a56db`, `#1e40af`, `#1e3a8a`, `#2563eb`, `#4f46e5` | `var(--accent)` — и для заливки, и для текста\*\* |
| `#ef4444`, `#dc2626` — **заливка / рамка / полоска / шкала** | `var(--danger)` |
| `#b91c1c`, `#991b1b`, `#7f1d1d` — **цвет текста** | `var(--danger-text)` |
| `#f59e0b` — **заливка / рамка / полоска / шкала** | `var(--warn)` |
| `#92400e`, `#b45309` — **цвет текста** | `var(--warn-text)` |
| `#10b981`, `#059669`, `#0e9f6e`\* — **заливка / рамка / полоска / шкала** | `var(--success)` |
| `#065f46`, `#047857`, `#166534` — **цвет текста** | `var(--success-text)` |
| `box-shadow: 0 2px 8px rgba(0,0,0,.08)` | `var(--shadow)` |

\* `#0e9f6e` — только там, где это НЕ фикс-цветная кнопка «КП» (см. ограничение 5).

\*\* `--accent-text` заводить НЕ нужно — посчитано: `#1a56db` даёт 6,18:1 на белом и 4,92:1 на тинте `--accent` 15%. Но запас нулевой: на подложке `--surface-2` выходит **4,50:1, ровно порог**. Значит accent-бейджи живут на `--surface`/`--surface-2` и не темнее — если понадобится посадить accent-текст на более тёмную подложку, сначала пересчитать контраст.

> ⚠️ **Заливочный и текстовый токены НЕ взаимозаменяемы — это две разные роли, а не синонимы.** Заливочный как цвет текста даёт **2,1–3,4:1** в светлой теме (так и уехало в прод: чипы стадий 1,91:1, «★ победитель» 2,13:1, ссылка назначения 2,54:1 — чинили задачей 5b). Текстовый как заливка сделает тинт мутным и грязным. В светлой теме это РАЗНЫЕ значения; в тёмной они совпадают (`--danger-text` == `--danger` и т.д.) — именно поэтому дефект и не был виден в тёмной теме, ради которой всё затевалось. Правило простое: смотри на слот CSS, а не на исходный хекс. `color:` → текстовый токен; `background`, `border-color`, `border-left`, `box-shadow`, заливка шкалы, любой `color-mix(...)` для подложки → заливочный.
>
> То же правило распространяется на **нейтральную пару** `#d1d5db`/`#e5e7eb`, а не только на семантические токены: в слоте `color` светло-серый хекс — это приглушённый ТЕКСТ (разделитель, очень бледная подпись), а не рамка. `--border` в тёмной теме — полупрозрачно-белый 11%, и глиф на нём просто исчезает (живой замер Task 6: разделитель «·» в шапке карточки оборудования получил `rgba(255,255,255,0.11)` и пропал). Поэтому для текста берётся `color-mix(in srgb, var(--text-muted) 45%, transparent)`: в светлой теме ≈`#b0b4bb` — та же бледность, что у `#d1d5db`, в тёмной ≈`#5f5c58` — видимо и ненавязчиво.
...
- **чип / бейдж / мелкий элемент:**
  `background: color-mix(in srgb, var(--TOKEN) 15%, transparent); color: var(--TOKEN-text);`
  (подложка — заливочный токен, надпись — текстовый; для `--accent` текстового варианта нет, там `color: var(--accent)`)
...
4. **Перевести** оставшиеся хексы по таблице выше.
   ...
   ⚠️ **Следи за ховерами, которые буквальный перевод превращает в no-op.** Если база и `:hover` были разными хексами, схлопывающимися в один токен (частый случай: `#f9fafb` и `#f3f4f6` — оба `--surface-2`), то дословный перевод оставит мёртвое правило, выглядящее осмысленным, а кнопка потеряет отклик. Бери существующие идиомы: `var(--accent-hover)` для акцентных, `color-mix(in srgb, var(--text) 12%, var(--surface-2))` для нейтральных. Новых красок не изобретать.

5. **Комментируй каждое место, где сознательно отступил от буквального перевода** (находка ревью Task 7). Без комментария следующий проход по таблице «починит» отступление обратно и молча сломает поведение — именно так едва не умер анти-no-op ховер в `applies`. Это касается: анти-no-op ховеров, сохранённой геометрии, доменных переопределений цвета, фикс-цветных заливок, всего, что «выглядит как недоперевод, но сделано нарочно».
6. Проверить, что `@media` остался последним блоком.
7. `cd frontend && npm run build` — зелёный.
...
### Task 5b: текстовые варианты семантических токенов (находка ревью Task 5)

**Зачем.** Таблица соответствия отправляет в ОДИН токен две разные роли: заливки (`#10b981`, `#ef4444`, `#f59e0b`) и **тёмные цвета текста** (`#065f46`, `#991b1b`, `#92400e`, `#166534`, `#047857`). Для заливки это верно. Как цвет текста на светлом тинте — просадка контраста в светлой теме: «★ победитель» **2,13:1** (было 3,32), `.oc-assign` **2,54:1** (было 3,77), текст алерта **3,39:1** (было ~9,5). В тёмной теме те же места нормальны.

Дефект уже уехал в прод вместе с тёмной темой 2026-07-19 и сейчас живёт в **37 местах** (`color: var(--danger|success|warn)`); из непереведённых компонентов придёт ещё **58** тёмных текстовых хексов. Чинить на уровне kit — одна правка; чинить после Волн 2–3 — около 95 мест.

**Files:**
- Modify: `frontend/src/styles.scss`
- Modify: уже переведённые компоненты, где семантический токен стоит как `color`

- [ ] **Step 1: Завести текстовые токены**

В `:root` — исходные тёмные значения (то есть светлая тема ВОЗВРАЩАЕТСЯ к виду до токенизации, это починка точности, а не редизайн). В тёмной — те же значения, что у заливочных токенов:

```scss
/* :root */
--danger-text:#991b1b; --success-text:#065f46; --warn-text:#92400e;
/* :root[data-theme="dark"] */
--danger-text:#f87171; --success-text:#34d399; --warn-text:#fbbf24;
```

- [ ] **Step 2: Обновить kit**

Во всех правилах kit, где семантический токен стоит как `color` (бейджи `.badge-*`, `.error-banner`, `.field-error`), заменить `color: var(--TOKEN)` на `color: var(--TOKEN-text)`. `background`/`border-color` НЕ трогать — там заливочный токен верен.

- [ ] **Step 3: Обновить уже переведённые компоненты**

Найти `grep -rn 'color: var(--\(danger\|success\|warn\))' frontend/src/app` и заменить на текстовый вариант — **только там, где это цвет текста**. Если токен стоит в `border-color`, `background`, `fill`, `stroke` — не трогать.

Отдельно проверить `.no-email` в `tenders.component.ts` и `.negative`/`.positive`, если они уже переведены.

- [ ] **Step 4: Обновить таблицу соответствия в этом плане**

Разделить строки: заливка/рамка → `--TOKEN`, текст → `--TOKEN-text`. Формула чипа становится:
`background: color-mix(in srgb, var(--TOKEN) 15%, transparent); color: var(--TOKEN-text);`
...
```
fix(ui): текстовые варианты семантических токенов — контраст в светлой теме

Таблица соответствия сливала в один токен заливки (#10b981/#ef4444) и тёмные
цвета текста (#065f46/#991b1b/#92400e). Как цвет текста на светлом тинте это
давало 2,13:1 у «★ победитель», 2,54:1 у ссылки назначения, 3,39:1 у текста
алерта — против 3,3–9,5:1 до токенизации.

--danger-text/--success-text/--warn-text в светлой теме равны исходным тёмным
значениям (то есть возвращают точность), в тёмной — заливочным токенам.
...
⚠️ **Развилка тени дровера (Minor из ревью Task 6), закрыть здесь один раз.** `.sidebar` в этом файле (`~строка 171`) дословно повторяет дровер из `equipment-detail-modal`, где тень перевели в `var(--shadow-lg)` (`0 12px 32px`, светит вниз), потеряв исходную направленность влево (`-8px 0 30px`). При этом в `layout.component.ts:235` направленную тень дровера при токенизации **оставили литералом**. То есть в репозитории сейчас два разных решения для одного случая.

Выбери одно и примени здесь же, а вторым коммитом приведи к нему `equipment-detail-modal`. Рекомендация: направленность у бокового дровера осмысленна (тень падает от края экрана), поэтому literal-путь честнее — оставить `-8px 0 30px` с токенизированной альфой либо завести `--shadow-drawer`. Что выберешь, то и запиши в отчёт.
