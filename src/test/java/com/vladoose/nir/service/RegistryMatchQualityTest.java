...
class RegistryMatchQualityTest {
    ...
    void reportsQualityMetrics() throws Exception {
        ...
        // 5b. И — отдельно — уверенность с ЧУЖОЙ записью в топе ОТВЕРГАЕТСЯ, а не просто
        //     не засчитывается. Отсечка 5 сама по себе конфигурацию не заворачивает: при
        //     MAX_RIVALS=8 «Трубка эндотрахеальная» становится CONFIDENT с записью
        //     «Эндотрахеальная трубка 5520» в топе, притом что ни один из четырёх размеченных
        //     эндобронхиальных ключей не попадает в топ-5. ruConfidentCorrect при этом
        //     остаётся 8 (8 >= 8 проходит), ни один NONE/GENERIC не переворачивается — и все
        //     шесть отсечек зеленеют на конфигурации, возвращающей третий режим отказа плана
        //     («класс верный, запись не та»). Обещание фичи — «не выглядеть увереннее, чем
        //     позволяют данные», поэтому уверенная ошибка обязана ронять сборку, а не молча
        //     не получать зачёта. Сегодня ровно 0, то есть отсечка бесплатна.
        int confidentButWrong = byExpectation.get(MatchConfidence.CONFIDENT)[2] - ruConfidentCorrect;
        assertThat(confidentButWrong)
                .as("РУ-лотов, названных уверенно, но с ЧУЖОЙ записью в топ-5")
                .isZero();
