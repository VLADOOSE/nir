...
- **Реестр-матч по разобранному ТЗ** (сейчас `candidatesForLot` матчит по куцему `equipName` даже после разбора): упирается в `word_similarity`/`<%` (порог 0.6) — он резко чувствителен к длине запроса (фраза >2–3 слов отсекается фильтром целиком). Нужна отдельная проработка: понижать `pg_trgm.word_similarity_threshold`/`set_limit` для лот-матча или извлекать именно наименование изделия (не весь абзац характеристик). Наивное «первые N слов» хрупко (N между 2 и 3 непредсказуем).
