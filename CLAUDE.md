...
- Хардкод-селект типов в форме лота (`tenders.component` форма: УЗИ/Рентген/ИВЛ/Монитор строкой, не сохраняется — `TenderLotRequest` ждёт `equipTypeId`) → справочник `equipment-types`. Дефект.
