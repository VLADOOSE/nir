...
- **Отправка КП — ч.3 (разбор ответов):** структурный парс ответа поставщика (цена/срок/модель → `PriceRequestItem.responsePrice`), автозаполнение КП, сравнение предложений между поставщиками. (ч.1 «подбор по виду МИ» и ч.2 «грамотная отправка+приём» — закрыты, §9.) Следующие: персонализация тела пер-дистрибьютор (когда появятся контактные лица); HTML-шаблон письма; авто-IMAP-поллинг вместо кнопки «Проверить ответы» при боевом ящике; вынести `tenders.component.scss` (запас anyComponentStyle ~40 B до 16 kB). **Хардинг `resend`:** сейчас не обновляет `sentAt` и читает LAZY `items` под OSIV (нет `@Transactional`) — при части 3 объединить: отдельный @Transactional-шаг ставит `sentAt=now` перед `dispatch` (сеть вне tx, §6) + JOIN FETCH items; + FE-гард «Переслать» от двойного клика (нет `sending`-флага).
