...
- **Статусы тендеров (T9 закрыт):** `mapStatus`: 210–245 → ACTIVE, 410/420/430 → CANCELLED, ≥250 → COMPLETED; плюс правило дедлайна в writer (площадка держит «Опубликовано» после дедлайна → локально COMPLETED). `TenderStatusScheduler` (ежедневно 00:05 + на старте, оба рынка по §6) закрывает просроченные ACTIVE; следующий импорт сверяет с площадкой (продлили — вернётся ACTIVE). Статус импорта: `GET /api/tenders/import-kz/status` → `{running, lastFinishedAt, lastRegion, lastSummary}` (in-memory в шедулере; двойной POST во время прогона отбивается «Импорт уже выполняется»). UI `/tenders`: строка «Обновлено N мин назад · создано X, обновлено Y» у кнопки, сортировка «Сначала новые» (дефолт) / «Скоро дедлайн», мини-чипы лотов на карточке (title = спека), спека в таблице лотов разворачивается кликом, бейдж/фильтр «Отменён».
